...
class sample(Base):
    ...
    nanohub_userid = Column(
        Integer, info={
            'verbose_name': 'Nanohub Submitter User ID'})
    ...
    def json_encodable(self):
        ...
        for p in params:
            json_dict[p] = {
                'value': getattr(
                    self, p), 'unit': getattr(
                    recipe, p).info['std_unit']}
        json_dict['preparation_steps'] = sorted([s.json_encodable(
        ) for s in self.preparation_steps if s.step is not None], key=lambda s: s["step"])
...
class preparation_step(Base):
    ...
    recipe_id = Column(
        Integer, ForeignKey(
            recipe.id), primary_key=True, info={
    ...
    def json_encodable(self):
        ...
        for p in params:
            json_dict[p] = {
                'value': getattr(
                    self, p), 'unit': getattr(
                    preparation_step, p).info['std_unit']}
...
class author(Base):
    ...
    id = Column(Integer, primary_key=True, info={'verbose_name': 'ID'})
    sample_id = Column(
        Integer, ForeignKey('sample.id'), info={
            'verbose_name': 'Sample ID'})
    raman_id = Column(Integer, ForeignKey('raman_set.id'),
...
class raman_set(Base):
    ...
    def json_encodable(self):
        ...
        json_dict["raman_spectra"] = [r.json_encodable()
                                      for r in self.raman_spectra]
        json_dict['d_to_g'] = {'value:': getattr(self, 'd_to_g'), 'unit': None}
        json_dict['gp_to_g'] = {
            'value:': getattr(
                self, 'gp_to_g'), 'unit': None}
        for p in params:
            json_dict[p] = {
                'value': getattr(
                    self, p), 'unit': getattr(
                    raman_spectrum, p).info['std_unit']}
...
class mdf_forge(Base):
    ...
    mdf_id = Column(
        String(32), primary_key=True, info={
            'verbose_name': 'MDF ID'})
