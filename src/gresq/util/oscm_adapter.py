...
class OSCMAdapter:
    ...
    def _send_confirmation_email(self, transaction_name=None, resource_name=None):
        ...
        if transaction_name is None or resource_name is None:
            out['msg'] = 'Please provide the transaction name and the resource name as the arguments'
            return out
        ...
        if self.session.is_auth:
            ...
            body += '<p> A new transaction: <strong><i>' + transaction_name + '</i></strong> has been sent to the resource <strong>' + resource_name + '</strong>. </p>'
    ...
    def get_resource_id(self, resource=None):
        ...
        if resource is None or '_id' not in resource.keys():
            out['msg'] = 'Please provide the resource data in the argument. Resource must have an _id key'
            return out
    ...
    def get_facility_id(self, facility=None):
        ...
        if facility is None or '_id' not in facility.keys():
            out['msg'] = 'Please provide the facility data in the argument. Facility must have an _id key'
            return out
    ...
    def get_queues(self, facility=None):
        ...
        if facility is None or 'configuration' not in facility.keys():
            out['msg'] = 'Please provide the facility data in the argument. Facility must have a configuration key'
            return out
    ...
    def get_user_transactions(self, version='light'):
        ...
        if self.session.is_auth:
            ...
            try:
                response_customer = self.session.get_transactions(
                'customer', version)
            except:
                out['msg'] = 'OSCM Server is currently down'
                return out
            ...
            try:
                response_provider = self.session.get_transactions(
                'provider', version)
            except:
                out['msg'] = 'OSCM Server is currently down'
                return out
    ...
    def get_transaction_id(self, transaction=None):
        ...
        if transaction is None or 'id' not in transaction.keys():
            out['msg'] = 'Please provide the transaction data in the argument. Transaction must have an id key'
            return out
    ...
    # tested!
    def submit_transaction(self, transaction_name=None, resource_id=None, job_data=None):
        ...
        if transaction_name is None or resource_id is None or job_data is None:
            out['msg'] = 'Please provide the transaction name, resource id and job id as arguments'
            return out
        ...
        if self.session.is_auth:
            ...
            try:
                ...
                resource_response = self.get_resource(resource_id) if job_data['type'] is 'resource' else self.get_facility(resource_id, path='_doc')
            ...
            provider_id = resource['owner']['_id'] if job_data['type'] is 'resource' else resource['owner_locator']['_id']
            ...
            # set instructions
            instructions = job_data['instructions'] if job_data['instructions'] else 'no special instructions for this job'
            ...
            resource_locator = {
                '_id': resource_id,
                'name': resource['configuration']['profile']['resourcename'] if job_data['type'] is 'resource' else resource['configuration']['profile']['facility_name'],
                'type': job_data['type']
            }
            ...
            if response['success']:
                ...
                condirmation_email = self._send_confirmation_email(transaction_name=transaction_name, resource_name=resource_locator['name'])
    ...
    def submit_file(self, transaction_id=None, my_path=None, my_filename=None):
        ...
        if transaction_id is None or my_path is None or my_filename is None:
            out['msg'] = 'Please provide the transaction id, the path and the filename as arguments'
            return out
        ...
        if self.session.is_auth:
            ...
            if transaction_response['success']:
                ...
                try:
                    response = self.session.submit_file(transaction_data, my_path, my_filename)
                except:
                    out['msg'] = 'OSCM Server is currently down'
                    return out
    ...
    def register_user(self, user=None):
        ...
        if response_new_user['success']:
            ...
            if response_event['success']:
                # send confirmation email
                is_email = self.session.send_confirmation_email(response_event['event']['_id'], user['email'])['success']
